...
baseDev=$(cat "/home/$userHome/.cache/usb/file/sys-dev")
currentDev=$(ls /sys/bus/usb/devices/ | tr '\n' '#')

for dev in /sys/bus/usb/devices/*
do
	localDev=$(echo "$dev" | cut -d/ -f6-)
	if [[ ! "$localDev" == *"usb"* ]] ; then
		cDev="$localDev#"
		if [[ ! $baseDev =~ $cDev ]] ; then
			echo "0" > "$dev/authorized"
			echo "$localDev" > "/home/$userHome/.cache/usb/file/newDev"
			sleep 1
		fi
	fi
done
